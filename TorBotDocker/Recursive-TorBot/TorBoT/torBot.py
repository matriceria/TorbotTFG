...
def main(conn=False):
    ...
    parser.add_argument("-p", "--printout",
                        action="store_true",
                        help="Prints the results onto stdout (might fail if due to encoding errors)")
    ...
    parser.add_argument("-r", "--recursive",
                        help = "Recursively crawl onion websites",
                        default = 10,
                        type = int)
    ...
    if args.url:
        ...
        # -m/--mail
        if args.mail:
            if getemails.getMails(html_content):
                print("Keyword encontrada")
            if args.save:
                savefile.saveJson('Links con keyword', args.url)
        # -i/--info
        elif args.info:
            ...
            (onion_links, clear_links, keyword_link) = getweblinks.get_links_recursive(soup=html_content,
                                                                         rec_links=args.recursive,
                                                                         live=args.live,
                                                                         ext=args.extension,
                                                                         printout=args.printout)
            if args.save:
                keyword_link=list(set(keyword_link))
                savefile.saveJson("OnionLinks", onion_links)
                savefile.saveJson("ClearLinks", clear_links)
                savefile.saveJson("Keyword", keyword_link)
        else:
            ...
            if getemails.getMails(html_content):
                print("Keyword encontrada")
                savefile.saveJson('Links con keyword', onion_links)
            if args.save:
                print("Hola")
                savefile.saveJson("OnionLinks", onion_links)
                savefile.saveJson("ClearLinks", clear_links)
    else:
        print("usage: torBot.py [-h] [-v] [--update] [-q] [-u URL] [-r NUM_OF_SEEDS] [-s] [-p] [-m] [-e EXTENSION] [-l] [-i]")
